...
class Polarimeter():
    ...
    def start(self):
        ...
        while self.run:
            ...
            self.lib.TLPAX_getStokesNormalized(self.instrumentHandle, scanID, byref(self.s1),byref(self.s2),byref(self.s3))
            # print("Azimuth [rad]: ", azimuth.value)
            # print("Ellipticity [rad]: ", ellipticity.value)
            # print("s1: ", s1)
            # print("s2: ", s2)
            # print("s3: ", s3)
            self.s1Queue.put(float(self.s1.value))
            self.s2Queue.put(float(self.s2.value))
            self.s3Queue.put(float(self.s3.value))
